...
def consolidar_tabelas_municipios(diretorio_origem=None):
    ...
    cabecalhos = [
        "tabela_origem", "nome_metrica", "faixa_etaria", "capital", "pc",
        ...
        if os.path.exists(caminho):
            try:
                ...
                for row_idx in range(inicio_dados, len(dados)):
                    ...
                    if municipio.startswith("Tabela"):
                        continue
                    
                    # Pular linhas que são descrições/cabeçalhos
                    if len(municipio) > 100 or "(%)" in municipio:
                        continue
                    ...
                    # Se não tem valores ou tem descrição onde deveria ter número, pular
                    if not tem_valores:
                        continue
    ...
    # Salvar arquivo
    output_path = os.path.join(diretorio_origem, "Tabelas_12_Consolidadas_Capitais.xlsx")
