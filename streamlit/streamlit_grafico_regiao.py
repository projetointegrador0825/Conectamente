...
def pagina_pizza_estados(df_estados: pd.DataFrame, df_capitais: pd.DataFrame) -> None:
    ...
    Todas as faixas etárias são sempre agregadas (média) — sem filtro.
    ...
    with col_esq:
        ...
        fig_rel = px.pie(
            ...
            names=col_local,
            hole=0.38,
            title=f"Participação relativa — {metrica_exibida}",
        )
    ...
    with col_dir:
        ...
        fig_abs = px.pie(
            ...
            names=col_local,
            hole=0.38,
            title=f"Prevalência real (pc%) — {metrica_exibida}",
        )
    ...
    with st.expander("📋 Ver tabela de dados desta seleção"):
        df_exibir = df_agrupado.rename(columns={
            ...
            "prevalencia": f"Prevalência média — {metrica_exibida} (%)",
        })
        ...
        st.dataframe(
            df_exibir.style.format(
                {f"Prevalência média — {metrica_exibida} (%)": "{:.2f}%"}
            ),
